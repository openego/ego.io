...
from sqlalchemy.dialects.postgresql import DOUBLE_PRECISION, TEXT, NUMERIC, INTEGER, BIGINT
...
t_buergen_test = Table(
    'buergen_test', metadata,
    Column('id', BigInteger, nullable=False, server_default=text("nextval('model_draft.buergen_test_id_seq'::regclass)")),
    Column('initiierung', Text),
    Column('stellungnahmen', Text),
    Column('quellen', Text),
    Column('aussendarstellung', Text),
    Column('name', Text),
    Column('region_key', Text),
    schema='model_draft'
)


class CandidateEvChargingPoint(Base):
    __tablename__ = 'candidate_ev_charging_points'
    __table_args__ = {'schema': 'model_draft'}

    geom = Column(Geometry)
    id = Column(Integer, primary_key=True, server_default=text("nextval('model_draft.candidate_ev_charging_points_id_seq'::regclass)"))
...
class EgoSimpleFeedinFull(Base):
    ...
    coastdat_id = Column(Text, primary_key=True, nullable=False)
    sub_id = Column(Text, primary_key=True, nullable=False)
