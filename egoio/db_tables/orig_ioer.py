...
class IoerUrbanShareIndustrial(Base):
    ...
    rid = Column(Integer, primary_key=True, server_default=text("nextval('orig_ioer.ioer_urban_share_industrial_rid_seq'::regclass)"))
    rast = Column(Raster)
...
class IoerUrbanShareIndustrialCentroid(Base):
    ...
    id = Column(Integer, primary_key=True, server_default=text("nextval('orig_ioer.ioer_urban_share_industrial_centroid_id_seq'::regclass)"))
    rid = Column(Integer)
